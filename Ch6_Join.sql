...
--별칭 사용 안할 경우
select * 
from EMPLOYEEEE, DEPARTMENT
where EMPLOYEEEE.dno = DEPARTMENT.dno
order by eno;
 ...
 * 문제가 발생하는 이유? (예)
 * EMPLOYEEd의 dno와 department의 dno : 동일한 이름(dno)과 데이터 유형(number(2))
...
select eno, ename, dno, dname, EMPLOYEE.manager -- 반드시 테이블 이름이나 별칭 사용하여 구분해야 함
from EMPLOYEE join DEPARTMENT
...
from EMPLOYEE join SALGRADE --별칭 사용 안함(중복되는 컬럼 자체가 없음)
...
select ename, salary, grade
from EMPLOYEE, SALGRADE --별칭 사용 안함(중복되는 컬럼 자체가 없음)
