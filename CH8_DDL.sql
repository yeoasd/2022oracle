...
SELECT TABLE_NAME, CONSTRAINT_NAME, CONSTRAINT_TYPE
FROM USER_CONSTRAINTS
WHERE TABLE_NAME IN ('EMPLOYEE', 'DEPARTMENT');-- 테이블명 대문자로 검색해야된다 
...
--사용자 : system 일 때 - 결과 500 row(SYS와 SYSTEM만, 사용자(HR) 포함된 상태로 결과가 나옴)
...
--6.3 DBA_ 데이터사전 : 데이터베이스의 모든 객체 조회 가능(DBA_는 시스템 접근 권한)
--시스템 관리와 관련된 뷰, DBA나 시스템 권한을 가진 사용자만 접근 가능 
--system 계정으로 접속 하여 DBA_데이터 사전을 보는데, 
--이 때  SYSTEM이   DBA_데이터 사전을 볼 수 있는 권한을 가졌으면 조회가 가능함
select* 
from DBA_TABLES
where owner IN('HR');--결과 나옴 (이유?SYSTEM 은 DBA_데이터사전을 조회할 권한이 있어서 접근 가능)

-- system접속을 끊고 hr 계정으로 접속 
 select* 
from DBA_TABLES
where owner IN('HR');--결과가 안나옴(이유?hr은 DBA_데이터사전을 조회할 권한이 없어서 접근 불가능)
